package net.journeyreforged.items.projectiles;
...
public class DiamondDaggerProjectile extends Item implements Vanishable {
    ...
    public DiamondDaggerProjectile(Item.Settings settings) {
        super(settings);
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (!world.isClient) {
            ...
            if (j == 0) {
                DiamondDaggerEntity diamondDaggerEntity = new DiamondDaggerEntity(world, (LivingEntity)playerEntity, stack);
                diamondDaggerEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0f, 2.5f + (float)j * 0.5f, 1.0f);
                if (playerEntity.getAbilities().creativeMode) {
                    diamondDaggerEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                }
                world.spawnEntity(diamondDaggerEntity);
                world.playSoundFromEntity(null, diamondDaggerEntity, SoundEvents.ITEM_TRIDENT_THROW, SoundCategory.PLAYERS, 1.0f, 1.0f);
                if (!playerEntity.getAbilities().creativeMode) {
        ...
        if (j > 0) {
            float diamondDaggerEntity = playerEntity.getYaw();
            float f = playerEntity.getPitch();
            float g = -MathHelper.sin(diamondDaggerEntity * ((float)Math.PI / 180)) * MathHelper.cos(f * ((float)Math.PI / 180));
            float h = -MathHelper.sin(f * ((float)Math.PI / 180));
            float k = MathHelper.cos(diamondDaggerEntity * ((float)Math.PI / 180)) * MathHelper.cos(f * ((float)Math.PI / 180));
